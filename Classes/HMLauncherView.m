...
- (void)addIcon:(HMLauncherIcon *)icon
{
    ...
    if (icon.canBeDragged) {
        [self launcherIcon:icon addLongPressGestureRecognizerWithDuration:[self.delegate longPressDuration] requireGestureRecognizerToFail:tapGestureRecognizer];
    }
...
- (void)startScrollTimerWithOffset:(NSInteger)offset
{
    ...
    if (self.scrollTimer != nil) {
        ...
        self.scrollTimer = [NSTimer scheduledTimerWithTimeInterval:[self.delegate scrollingTimeInterval] target:self selector:@selector(executeScroll:) userInfo:nil repeats:NO];
    }
}
...
{
    CGSize iconSize = [self.dataSource buttonDimensionsInLauncherView:self];
    CGFloat springWidth = iconSize.width * [self.delegate scrollingFraction];
...
- (void)makeIconDraggable:(HMLauncherIcon *)icon
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
         icon.transform = CGAffineTransformMakeScale([self.delegate selectedZoomTransform], [self.delegate selectedZoomTransform]);
         icon.alpha = 0.9;
     }];
...
- (void)startShaking
{
    CGFloat rotation = ([self.delegate shakeRadian] * M_PI) / 180.0;
    ...
    [UIView animateWithDuration:[self.delegate shakeTime]
                          delay:0
